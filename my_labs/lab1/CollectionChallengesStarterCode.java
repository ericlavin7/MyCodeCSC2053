...
public class CollectionChallengesStarterCode {
	...
	public static void removeRange(ArrayList<Integer> list, int min, int max) {
		for (int counter = 0; counter < list.size(); ++counter) {
			if (list.get(counter) >= min && list.get(counter) <= max) {
				list.remove(counter);
				--counter;
				//must take into account that the element to the right of the one being removed will move one spot to the left
			}
		}
	...
	public static void addStars(ArrayList<String> vec) {
		for (int counter = 0; counter < vec.size()+1; ++counter) {
			vec.add(counter, "*");
			++counter;
		}
	...
	public static Map<String, String> topping1(Map<String, String> map) {
		...
		//check if map contains the key "ice cream"
